...
- [x] T007 Add `scripts/run-tests.sh` with one shared initialization sequence: `INSTALL/LOAD httpfs` and `INSTALL/LOAD ducklake`.
- [x] T008 Use a clean HOME in every job and print `duckdb_extensions()` before tests.
- [x] T009 Fail immediately if either extension cannot be installed or loaded.
