...
def validate_probe(
    ...
    # The command log is already CSV because DuckDB stdout is redirected there.
    subprocess.run(
        [sys.executable, str(SCRIPT_DIR / "validate-extension-probe.py"), str(extension_csv), str(extensions_json)],
        env=env,
        check=True,
    )
...
def main() -> int:
    ...
    runner_temp = Path(os.environ.get("RUNNER_TEMP", str(REPOSITORY_ROOT / "build" / "runtime"))).resolve()
    ...
    try:
        ...
        if os.name == "nt" and any(profile.get("services") for profile in profiles):
            names = [profile["name"] for profile in profiles if profile.get("services")]
            raise RunnerError(
                "profile-scoped WSL service bridge is required before native Windows execution: "
                + ",".join(names)
            )
        ...
        for source in (
            "battery.json", "extensions.json", "services.json", "prerequisites.json",
            "capabilities.json", "profiles.json", "install-extensions.sql", "init-extensions.sql",
        ):
        ...
        local_repo = prepare_local_extension_repo(extension_dir, runtime_root, duckdb_version, logger)
        ...
        for profile_name, test_filter in profile_rows(runtime_config):
            ...
            run_logged(
                [
                    str(unittest),
                    "--test-config", str(config),
                    "--test-dir", str(upstream_root),
                    test_filter,
                ],
                logger=logger,
            ...
            subprocess.run(
                [sys.executable, str(SCRIPT_DIR / "check-test-requirements.py"), str(unittest_log), str(extensions_json)],
                env=env,
                check=True,
            )
        ...
        return 0
    except (OSError, KeyError, json.JSONDecodeError, RunnerError, subprocess.CalledProcessError) as exc:
